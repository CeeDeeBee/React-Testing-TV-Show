...
test("changing the season dropdown renders episodes", async () => {
	...
	const dropdownOptions = queryAllByRole(/option/i);
	// All dropdown options render
	expect(dropdownOptions).toHaveLength(4);
	userEvent.click(dropdownOptions[0]);
	// All episodes render for slected dropdown
	expect(queryAllByTestId(/episode/i)).toHaveLength(8);

	// Can switch season again
	userEvent.click(queryAllByText(/season 1/i)[0]);
	userEvent.click(queryAllByRole(/option/i)[1]);
	expect(queryAllByTestId(/episode/i)).toHaveLength(9);
});
